...
>提示：当`Aurelia.prototype.setRoot`完成时，app主机元素的内容（标记为`aurelia-app`或传递给`Aurelia.prototype.setRoot`的内容）将被替换。


>警告：当使用`<body>`元素作为应用程序宿主时，请记住在完成`Aurelia.prototype.setRoot`之前添加的任何内容都将被删除。
